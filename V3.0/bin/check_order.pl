...
### SET PATHS ###
my ($bindir, $maindir, $dbdir, $exedir, $rdir, $orderdir);
...
if ($fail ne "fail") {
	cluster_go();
	report();
	plot("$orderdir/order.patterntable.txt", "$orderdir/order.patterns.pdf");
#	plot("$orderdir/order.speciestable.txt", "$orderdir/order.species.pdf");
}

unlink("plot_order.Rout");
unlink("$orderdir/order.patterntable.txt");
unlink("$orderdir/order.speciestable.txt");

system("touch $outdir/order.success");
...
sub report {
	my $i = 0;
	open OUT1, ">$orderdir/order.species.txt";
	open OUT2, ">$orderdir/order.patterns.txt";
	open OUT3, ">$orderdir/order.detailed.txt";
	open OUT4, ">$orderdir/order.patterntable.txt";
	open OUT5, ">$orderdir/order.speciestable.txt";
