...
# Make a file that will send instructions to Clustal with appropriate name of input file
	...
	if ($pauser eq "clustalw") {
		...
		$muscleoutfile=$out_file . '.aa_ali.fasta';
		my $scriptdir;
		($scriptdir = $0) =~ s/translatorx_vLocal.edit.pl//g;
		print STDERR "$scriptdir/../mafft-7.222-with-extensions/bin/"."mafft --auto $muscleinfile > $muscleoutfile\n";
		open(LOG, ">$out_file.$pauser.log") || die "Cannot create file outfile.pauser.log: $out_file.$pauser.log\n";
		my $pid = open(PH, "$scriptdir/../mafft-7.222-with-extensions/bin/"."mafft --auto $muscleinfile 2>&1 1>$muscleoutfile |");              # or with an open pipe
		while(<PH>) {
			print STDERR $_;
			print LOG $_;
		}
		...
		my $scriptdir;
		print STDERR "$scriptdir/../prank-msa/src/"."prank -d=$clustalinfile -o=$clustalinfile -F -noxml -notree\n";
		open(LOG, ">$out_file.$pauser.log") || die "Cannot create file outfile.pauser.log: $out_file.$pauser.log\n";
		my $pid = open(PH, "$scriptdir/../prank-msa/src/"."prank -d=$clustalinfile -o=$clustalinfile -F -noxml -notree 2>&1 |");              # or with an open pipe
		while(<PH>) {
			print STDERR $_;
			print LOG $_;
		}
...
if($gblocks_params ne "") {
	...
	my $scriptdir;
	($scriptdir = $0) =~ s/translatorx_vLocal.edit.pl//g;
	system "$scriptdir/../Gblocks_0.91b/"."Gblocks $aa_ali_file $gblocks_params";
	system "$scriptdir/../Gblocks_0.91b/"."Gblocks $aa_ali_file $gblocks_params -p=s";
